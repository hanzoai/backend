 ...
 * @returns hanzoUser object
 */
export const hanzoUser = (currentUser: User, data?: Record<string, any>) => {
...
/** Omits internal `_hanzo_*` fields for writing to the database */
export const omitHanzoFields = <T = Record<string, any>>(row: T) => {
  const shallowClonedRow: any = { ...row };
  delete shallowClonedRow["_hanzo_ref"];
  delete shallowClonedRow["_hanzo_outOfOrder"];
  delete shallowClonedRow["_hanzo_missingRequiredFields"];
  delete shallowClonedRow["_hanzo_new"];

  Object.keys(shallowClonedRow).forEach((key) => {
    if (key.startsWith("_hanzo_formulaValue_")) {
 ...
 * for Hanzo Run `buildFunction` endpoint.
 * Hanzo Run expects the previous URL format for sub-tables.
