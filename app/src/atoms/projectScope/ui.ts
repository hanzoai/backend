...
export const navOpenAtom = atomWithStorage("__HANZO__NAV_OPEN", false);
...
export type HanzoRunModalState = {
  open: boolean;
  feature: string;
  version: string;
};
/**
 * Open global Hanzo Run modal if feature not available.
 * Calling the set function resets props.
 *
 * @example Basic usage:
 * ```
 * const openHanzoRunModal = useSetAtom(hanzoRunModalAtom, projectScope);
 * openHanzoRunModal({ feature: ... , version: ... });
 * ```
 *
 * @example Close dialog:
 * ```
 * openHanzoRunModal({ open: false })
 * ```
 */
export const hanzoRunModalAtom = atom(
  { open: false, feature: "", version: "" } as HanzoRunModalState,
  (_, set, update?: Partial<HanzoRunModalState>) => {
    set(hanzoRunModalAtom, {
      open: true,
      feature: "",
      version: "",
      ...update,
...
export const tableOutOfOrderDismissedAtom = atomWithStorage(
  "__HANZO__OUT_OF_ORDER_TOOLTIP_DISMISSED",
  false
);
