...
const tableRowsLocalReducer = (
  ...
  switch (action.type) {
    ...
    case "update":
      const index = findIndex(prev, ["_hanzo_ref.path", action.path]);
      if (index > -1) {
    ...
    case "delete":
      return prev.filter((row) => {
        if (Array.isArray(action.path)) {
          return !action.path.includes(row._hanzo_ref.path);
        } else {
          return row._hanzo_ref.path !== action.path;
        }
      });
...
export const tableRowsAtom = atom<TableRow[]>((get) => {
  ...
  rowsDb.forEach((row, i) => rowsDbMap.set(row._hanzo_ref.path, i));
  ...
  const rowsLocalToMerge = rowsLocal.map((row, i) => {
    ...
    if (rowsDbMap.has(row._hanzo_ref.path)) {
      const index = rowsDbMap.get(row._hanzo_ref.path)!;
      const merged = updateRowData({ ...rowsDb[index] }, row);
      rowsDbMap.delete(row._hanzo_ref.path);
      return merged;
    }
    return row;
  });
  ...
  return [
    ...rowsLocalToMerge,
    ...rowsDb.filter((row) => rowsDbMap.has(row._hanzo_ref.path)),
  ];
});
 ...
 * Silently fails if auditing is disabled for the table or Hanzo Run version
 * not compatible.
 *
