...
export const addRowAtom = atom(
  ...
  async (get, set, { row, ignoreRequiredFields, setId }: IAddRowOptions) => {
    ...
    const _addSingleRowAndAudit = async (row: TableRow) => {
      ...
      const initialValues: TableRow = { _hanzo_ref: row._hanzo_ref };
      ...
      if (tableSettings.audit !== false) {
        const auditValue = hanzoUser(currentUser);
      ...
      if (
        row._hanzo_outOfOrder === true ||
        outOfOrderFilters.size > 0 ||
        setId !== "decrement"
      ) {
        set(tableRowsLocalAtom, {
          type: "add",
          row: { ...rowValues, _hanzo_outOfOrder: true },
        });
      }
      ...
      // (not out of order since those cases are handled above)
        ...
        set(tableRowsLocalAtom, {
          type: "add",
          row: { ...rowValues, _hanzo_outOfOrder: false },
        });
      }

      // Write to database if no required fields are missing
      else {
        await updateRowDb(row._hanzo_ref.path, omitHanzoFields(rowValues));
      }

      if (auditChange) auditChange("ADD_ROW", row._hanzo_ref.path);
    };

    // Find the first row in order to be used to decrement ID
    let firstInOrderRowId = tableRows[0]?._hanzo_ref.id;
    for (const row of tableRows) {
      if (row._hanzo_outOfOrder === false) {
        firstInOrderRowId = row._hanzo_ref.id;
        break;
      }
    }
    ...
    if (Array.isArray(row)) {
      ...
      for (const r of row) {
        ...
        const path = setId
          ? `${r._hanzo_ref.path.split("/").slice(0, -1).join("/")}/${id}`
          : r._hanzo_ref.path;

        promises.push(
          _addSingleRowAndAudit(setId ? { ...r, _hanzo_ref: { id, path } } : r)
        );
      }
      ...
      const path = setId
        ? `${row._hanzo_ref.path.split("/").slice(0, -1).join("/")}/${id}`
        : row._hanzo_ref.path;

      await _addSingleRowAndAudit(
        setId ? { ...row, _hanzo_ref: { id, path } } : row
      );
    }
  }
);
  ...
  async (
    ...
    const _deleteSingleRowAndAudit = async (path: string) => {
      const isLocalRow = Boolean(
        find(tableRowsLocal, ["_hanzo_ref.path", path])
      );
  ...
  async (
    ...
    const operations = rows.map((row) => ({
      type: type
        ...
        ? ("update" as "update")
        : ("add" as "add"),
      path: `${collection}/${row?._hanzo_ref?.id ?? generateId()}`,
      data: { ...initialValues, ...omitHanzoFields(row) },
    }));
  ...
  async (
    ...
    const row = find(
      ...
      arrayTableData?.index !== undefined
        ? ["_hanzo_ref.arrayTableData.index", arrayTableData?.index]
        : ["_hanzo_ref.path", path]
    );

    if (!row) throw new Error("Could not find row");
    const isLocalRow =
      fieldName.startsWith("_hanzo_formulaValue_") ||
      Boolean(find(tableRowsLocal, ["_hanzo_ref.path", path]));
    ...
    if (isLocalRow) {
      ...
      //  const [, key] = fieldName.split("_hanzo_formulaValue_");
      //  return c.key === key;
      // });
      // if(!config.persist) return;
      if (fieldName.startsWith("_hanzo_formulaValue")) return;
