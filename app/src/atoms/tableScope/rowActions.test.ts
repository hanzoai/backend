...
const initRows = (
  ...
  renderHook(async () => {
    ...
    setUpdateRowDb(() => async (path: string, update: Partial<TableRow>) => {
      setRowsDb((rows) => {
        const index = findIndex(rows, ["_hanzo_ref.path", path]);
        ...
        if (index === -1) {
          return sortBy(
            [
              ...rows,
              { ...update, _hanzo_ref: { id: path.split("/").pop()!, path } },
            ],
            ["_hanzo_ref.id"]
          );
        }
...
const generatedRows = new Array(GENERATED_ROWS_LENGTH)
  .fill(undefined)
  .map((_, i) => ({
    _hanzo_ref: { id: `row${i}`, path: TEST_COLLECTION + "/row" + i },
    index: i,
  }));
const generatedRowsLocal = new Array(GENERATED_ROWS_LENGTH)
  .fill(undefined)
  .map((_, i) => ({
    _hanzo_ref: { id: `rowLocal${i}`, path: TEST_COLLECTION + "/rowLocal" + i },
    index: i,
  }));
...
describe("addRow", () => {
  describe("single", () => {
    test("adds a single row with pre-defined id", async () => {
      ...
      await act(() =>
        addRow({
          row: {
            _hanzo_ref: { id: "addedRow", path: TEST_COLLECTION + "/addedRow" },
            added: true,
          },
        })
      );
      ...
      expect(find(tableRows, ["_hanzo_ref.id", "addedRow"])).toBeDefined();
      expect(find(tableRows, ["_hanzo_ref.id", "addedRow"])?.added).toBe(true);
    });
    ...
    test("adds a single row and generates random id", async () => {
      ...
      expect(tableRows).toHaveLength(GENERATED_ROWS_LENGTH + 1);
      expect(find(tableRows, ["_hanzo_ref.id", "addedRow"])).toBeUndefined();
      expect(find(tableRows, ["added", true])).toBeDefined();
      expect(find(tableRows, ["added", true])?._hanzo_ref.id).toHaveLength(20);
    });
    ...
    test("adds a single row and decrements id", async () => {
      ...
      expect(find(tableRows, ["_hanzo_ref.id", "addedRow"])).toBeUndefined();
      expect(find(tableRows, ["added", true])).toBeDefined();
      expect(find(tableRows, ["added", true])?._hanzo_ref.id).toBe(
        decrementId("row0")
      );
    });
  ...
  // TODO: TEST _hanzo_* fields are removed
  ...
  describe("multiple", () => {
    test("adds multiple rows with pre-defined id", async () => {
      ...
      expect(find(tableRows, ["_hanzo_ref.id", "addedRow0"])).toBeDefined();
      expect(find(tableRows, ["_hanzo_ref.id", "addedRow0"])?.added).toBe(true);
      expect(find(tableRows, ["_hanzo_ref.id", "addedRow1"])).toBeDefined();
      expect(find(tableRows, ["_hanzo_ref.id", "addedRow1"])?.added).toBe(true);
      expect(find(tableRows, ["_hanzo_ref.id", "addedRow2"])).toBeDefined();
      expect(find(tableRows, ["_hanzo_ref.id", "addedRow2"])?.added).toBe(true);
    });
    ...
    test("adds multiple rows and generates random id", async () => {
      ...
      expect(tableRows).toHaveLength(GENERATED_ROWS_LENGTH + 3);
      expect(find(tableRows, ["_hanzo_ref.id", "addedRow0"])).toBeUndefined();
      expect(find(tableRows, ["_hanzo_ref.id", "addedRow1"])).toBeUndefined();
      expect(find(tableRows, ["_hanzo_ref.id", "addedRow2"])).toBeUndefined();
      expect(find(tableRows, ["added", true])).toBeDefined();
      expect(find(tableRows, ["added", true])?._hanzo_ref.id).toHaveLength(20);
    });
...
describe("deleteRow", () => {
  test("deletes a single row", async () => {
    ...
    expect(tableRows).toHaveLength(GENERATED_ROWS_LENGTH - 1);
    expect(find(tableRows, ["_hanzo_ref.id", "row2"])).toBeFalsy();
  });
  ...
  test("deletes a single local row", async () => {
    ...
    expect(find(tableRows, ["_hanzo_ref.id", "rowLocal2"])).toBeFalsy();
  });
  ...
  test("deletes multiple rows", async () => {
    ...
    expect(tableRows).toHaveLength(GENERATED_ROWS_LENGTH - 3);
    expect(find(tableRows, ["_hanzo_ref.id", "row1"])).toBeFalsy();
    expect(find(tableRows, ["_hanzo_ref.id", "row2"])).toBeFalsy();
    expect(find(tableRows, ["_hanzo_ref.id", "row8"])).toBeFalsy();
  });
  ...
  test("deletes all rows", async () => {
    ...
    await act(() =>
      deleteRow({
        path: generatedRows.map((row) => row._hanzo_ref.path),
      })
    );
