...
import { omitHanzoFields } from "@src/utils/table";
    ...
    (index: number): UpdateFunction<T> => {
      setData((prevData) => {
        ...
        for (let i = index; i < prevData.length; i++) {
          // @ts-ignore
          prevData[i]._hanzo_ref.arrayTableData.index = i;
        }
        return sortRows(prevData, sorts);
      });
      return (rows) => {
...
function unsortRows<T = TableRow>(rows: T[]): T[] {
  return orderBy(rows, ["_hanzo_ref.arrayTableData.index"], ["asc"]);
}
