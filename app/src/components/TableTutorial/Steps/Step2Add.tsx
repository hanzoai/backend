...
export const Step2Add = {
  ...
  description:
    "When you make changes made to your data in Hanzo, they’re reflected in your Firestore database in realtime.",
  StepComponent,
  completeText: (
    <Typography variant="body1">
      <strong>Nicely done!</strong> Rating is just one of Hanzo’s 30+ field
...
function StepComponent({ setComplete }: ITableTutorialStepComponentProps) {
  ...
  useEffect(() => {
    if (tableRows.length >= 6) {
      ...
      const { _hanzo_ref, ...firstRow } = tableRows[0];
      if (!isEmpty(firstRow)) {
