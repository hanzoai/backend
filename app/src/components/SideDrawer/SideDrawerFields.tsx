...
export default function SideDrawerFields({ row }: ISideDrawerFieldsProps) {
  ...
  return (
    <Stack spacing={3}>
      ...
      <FieldWrapper
        type="debug"
        fieldName="_hanzo_ref.path"
        label="Document path"
        debugText={
          row._hanzo_ref.arrayTableData
            ? row._hanzo_ref.path +
              " → " +
              row._hanzo_ref.arrayTableData.parentField +
              "[" +
              row._hanzo_ref.arrayTableData.index +
              "]"
            : row._hanzo_ref.path
        }
        debugValue={row._hanzo_ref.path ?? row._hanzo_ref.id ?? "No ref"}
      />
