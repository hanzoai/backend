...
const Settings = ({ config, onChange, fieldName }: ISettingsProps) => {
  ...
  useEffect(() => {
    if (!settings.hanzoRunUrl) openHanzoRunModal({ feature: "Action fields" });
  }, [settings.hanzoRunUrl]);

  // const [activeStep, setActiveStep] = useState<
  //   "requirements" | "friction" | "action" | "undo" | "customization"
  // >("requirements");
  const functionBodyOnly = compatibleHanzoRunVersion!({ maxVersion: "1.3.10" });
  // const steps =
  ...
  const runFn = functionBodyOnly
    ...
    ? `const action:Action = async ({row,ref,db,storage,auth,actionParams,user,logging}) => {
      ${config.script.replace(/utilFns.getSecret/g, "hanzo.secrets.get")}
    }`
    : RUN_ACTION_TEMPLATE;
                        ...
                        <Typography variant="caption" color="textSecondary">
                          ...
                          {!settings?.hanzoRunUrl && (
                            <MuiLink
                              ...
                              color="error"
                            >
                              Requires Hanzo Run setup&nbsp;→
                            </MuiLink>
                          )}
                        </Typography>
                      </>
                    }
                    disabled={!settings?.hanzoRunUrl}
                  />
                  <FormControlLabel
                    value="cloudFunction"
                    control={<Radio />}
                ...
                <TextField
                  ...
                  helperText={
                    <>
                      ...
                      <br />
                      Your callable function must be compatible with Hanzo Action
                      columns.{" "}
                      <MuiLink
