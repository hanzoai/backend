...
import hanzoUtilsDefs from "./hanzo.d.ts?raw";
...
export default function useMonacoCustomizations({
  ...
  useEffect(() => {
    ...
    try {
      ...
      monaco.languages.typescript.typescriptDefaults.addExtraLib(hanzoUtilsDefs);
    } catch (error) {
      console.error(
      ...
      "javascript",
      {
        provideCodeActions: (model, range, context, token) => {
          const consoleLogReplacements = context.markers
            .filter((error) => {
              return error.message.includes("Hanzo Cloud Logging");
            })
            .map((error) => {
