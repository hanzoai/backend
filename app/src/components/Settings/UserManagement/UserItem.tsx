...
export default function UserItem({
  ...
  const hasHanzoRun = !!projectSettings.hanzoRunUrl;

  const handleSave = async () => {
    if (!hasHanzoRun) {
      openHanzoRunModal({ feature: "User Management" });
      return;
    }
    try {
      ...
      if (res.success) {
        ...
        await updateUser(_hanzo_ref!.path, { roles: value });
  ...
  const listItemChildren = (
    <>
      <ListItemAvatar>
        <EmojiAvatar
          src={user?.photoURL}
          fallback={user?.displayName || _hanzo_ref?.id || "?"}
        />
      </ListItemAvatar>
      <ListItemText
        ...
        <>
          ...
          <Tooltip title="Copy UID">
            <IconButton
              aria-label="Copy UID"
              onClick={async () => {
                if (!_hanzo_ref?.id) return;
                await navigator.clipboard.writeText(_hanzo_ref.id);
                enqueueSnackbar(
                  `Copied UID for ${user?.email}: ${_hanzo_ref.id}`
                );
              }}
            >
              <CopyIcon />
