  ...
  const {
    ...
    estimateSize: useCallback(
      (index: number) =>
        ...
        (tableRows[index]._hanzo_outOfOrder ? OUT_OF_ORDER_MARGIN : 0),
  ...
  const {
    ...
    estimateSize: useCallback(
      (index: number) => {
        ...
        if (definedWidth < MIN_COL_WIDTH && columnDef.id !== "_hanzo_select")
  ...
  useEffect(() => {
    ...
    if (selectedCell.path) {
      const rowIndex = tableRows.findIndex(
        (row) => row._hanzo_ref.path === selectedCell.path
      );
