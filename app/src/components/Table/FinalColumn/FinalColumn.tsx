...
export const FinalColumn = memo(function FinalColumn({
  ...
  const handleDelete = () => {
    const _delete = () =>
      deleteRow({
        path: row.original._hanzo_ref.path,
        options: row.original._hanzo_ref.arrayTableData,
      });
    if (altPress || row.original._hanzo_ref.arrayTableData !== undefined) {
      _delete();
    } else {
      confirm({
        title: "Delete row?",
  ...
  const handleDuplicate = () => {
    const _duplicate = () => {
      if (row.original._hanzo_ref.arrayTableData !== undefined) {
        if (!updateRowDb) return;

        return updateRowDb("", {}, undefined, {
          index: row.original._hanzo_ref.arrayTableData.index,
          operation: {
            addRow: "bottom",
            base: row.original,
          },
    ...
    >
      ...
      <Tooltip title={`Delete row${altPress ? "" : "…"}`}>
        <IconButton
          ...
          disabled={!row.original._hanzo_ref.path}
        >
          <DeleteIcon />
        </IconButton>
      </Tooltip>
