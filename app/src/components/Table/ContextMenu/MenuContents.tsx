...
export default function MenuContents({ onClose }: IMenuContentsProps) {
  ...
  const row = find(
    ...
    selectedCell?.arrayIndex === undefined
      ? ["_hanzo_ref.path", selectedCell.path]
      : // if the table is an array table, we need to use the array index to find the row
        ["_hanzo_ref.arrayTableData.index", selectedCell.arrayIndex]
  );
  ...
  const handleDuplicate = () => {
    const _duplicate = () => {
      if (row._hanzo_ref.arrayTableData !== undefined) {
        if (!updateRowDb) return;

        return updateRowDb("", {}, undefined, {
          index: row._hanzo_ref.arrayTableData.index,
          operation: {
            addRow: "bottom",
            base: row,
          },
    ...
    if (altPress || row._hanzo_ref.arrayTableData !== undefined) {
      _duplicate();
    } else {
      confirm({
        title: "Duplicate row?",
  ...
  const handleDelete = () => {
    const _delete = () =>
      deleteRow({
        path: row._hanzo_ref.path,
        options: row._hanzo_ref.arrayTableData,
      });
  ...
  const rowActions: IContextMenuItem[] = [
    {
      ...
      icon: <CopyIcon />,
      onClick: () => {
        navigator.clipboard.writeText(row._hanzo_ref.id);
        onClose();
      },
    },
    {
      ...
      icon: <CopyIcon />,
      onClick: () => {
        navigator.clipboard.writeText(row._hanzo_ref.path);
        onClose();
      },
    },
    {
      ...
      onClick: () => {
        window.open(
          `https://console.firebase.google.com/project/${projectId}/firestore/data/~2F${row._hanzo_ref.path.replace(
            /\//g,
            "~2F"
          )}`
        );
